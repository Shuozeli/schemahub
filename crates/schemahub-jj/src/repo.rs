...
//! `(project, repo)` and bridges jj-lib's async API to schemahub's sync `Jj`.
//!
...
//!   `DefaultSubmoduleStore`, pointed at a per-`Jj` temp directory. The index
...
//! reactor IO or timers). The `Jj` public API is synchronous, so we drive each
...
//! when a `Jj` method is called from inside the server's tokio runtime, whereas
...
pub(crate) struct Store {
    ...
    /// Per-`Jj` scratch dir for jj's index/submodule caches (reconstructable).
    index_root: PathBuf,
}
...
impl Store {
    ...
    pub(crate) fn loader(&self, repo_key: &str) -> JjResult<RepoLoader> {
        ...
        let jj_store = JjStore::new(
            Box::new(backend),
            Signer::from_settings(&self.settings).map_err(|e| JjError::Other(e.to_string()))?,
            merge_options,
        );
        ...
        std::fs::create_dir_all(&index_dir).map_err(|e| JjError::Other(e.to_string()))?;
        let index_store: Arc<dyn jj_lib::index::IndexStore> = Arc::new(
            DefaultIndexStore::init(&index_dir).map_err(|e| JjError::Other(e.to_string()))?,
        );
    ...
    pub(crate) fn load_repo(&self, repo_key: &str) -> JjResult<Arc<ReadonlyRepo>> {
...
impl From<ObjectDbError> for JjError {
    fn from(e: ObjectDbError) -> Self {
        match e {
            ObjectDbError::NotFound => JjError::ObjectNotFound,
            other => JjError::ObjectDb(other),
        }
    }
}
