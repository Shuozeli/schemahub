        ...
        auth: AuthConfig {
            ...
            tokens: HashMap::from([
                ...
                (
                    ...
                    TokenIdentity { id: "maeve".to_string(), display: None },
                ),
                (
                    "writer-token".to_string(),
                    TokenIdentity { id: "wendy".to_string(), display: None },
                ),
                (
                    "reader-token".to_string(),
                    TokenIdentity { id: "ron".to_string(), display: None },
                ),
                (
                    "stranger-token".to_string(),
                    TokenIdentity { id: "stranger".to_string(), display: None },
                ),
            ]),
        },
        ..Default::default()
...
async fn case2_public_project_anonymous_can_read_not_write() {
    ...
    assert!(read.is_ok(), "anonymous read on public project: {:?}", read.err());
...
async fn case3_private_project_roles_gate_read_and_write() {
    ...
    let reader_read = c.explore.list_declarations(with_token(
    ...
    let reader_write = c.schema.create_schema(with_token(
        Request::new(pb::CreateSchemaRequest {
            project: "acme".into(),
            repo: "core".into(),
            branch: "feature".into(),
    ...
    let writer_write = c.schema.create_schema(with_token(
        Request::new(pb::CreateSchemaRequest {
            project: "acme".into(),
            repo: "core".into(),
            branch: "feature".into(),
    ...
    .await;
    assert!(writer_write.is_ok(), "writer write: {:?}", writer_write.err());
}
...
async fn case4_force_on_protected_branch_requires_maintainer() {
    ...
    let writer_force = c.schema.apply_mutation(with_token(
    ...
    let maint_force = c.schema.apply_mutation(with_token(
        Request::new(pb::ApplyMutationRequest {
            project: "acme".into(),
            repo: "core".into(),
            branch: "main".into(),
    ...
    .await;
    assert!(maint_force.is_ok(), "maintainer force: {:?}", maint_force.err());
}
...
async fn case5_member_management_is_owner_only_with_zero_owner_guard() {
    ...
    let non_owner = c.project.add_member(with_token(
        Request::new(pb::AddMemberRequest {
            project: "acme".into(),
    ...
    let solo_remove = c.project.remove_member(with_token(
    ...
    let downgrade = c.project.update_member_role(with_token(
    ...
    let remove_alice = c.project.remove_member(with_token(
        Request::new(pb::RemoveMemberRequest {
            project: "acme".into(),
            identity: "alice".into(),
        }),
    ...
    .await;
    assert!(remove_alice.is_ok(), "remove alice once 2 owners: {:?}", remove_alice.err());
}
...
async fn case6_create_project_makes_caller_the_owner() {
    ...
    let add = c.project.add_member(with_token(
        Request::new(pb::AddMemberRequest {
...
async fn case7_list_projects_filters_by_visibility_and_membership() {
    ...
    let anon_list = c
        .project
        .list_projects(pb::ListProjectsRequest { name_prefix: String::new() })
        .await
    ...
    let ron_list = c
        ...
        .list_projects(with_token(
            Request::new(pb::ListProjectsRequest { name_prefix: String::new() }),
            "reader-token",
        ))
        .await
        .expect("ron list")
    ...
    let mut ron_names: Vec<String> =
        ron_list.projects.iter().map(|p| p.name.clone()).collect();
    ron_names.sort();
    assert_eq!(ron_names, vec!["public-proj".to_string(), "ron-priv".to_string()]);
    ...
    let mut alice_names: Vec<String> =
        alice_list.projects.iter().map(|p| p.name.clone()).collect();
    alice_names.sort();
    assert_eq!(alice_names, vec!["alice-only".to_string(), "public-proj".to_string()]);
}
